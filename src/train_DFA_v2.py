...
def train_dfa(X, Y, num_feats, num_pdfs, epochs=20):
    ...
    train_n = int(train_ratio * n_samples)
    ...
    model = DFA_MLP(num_feats, num_pdfs, hidden_dim=hidden_dim).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(0, X_train.size(0), batch_size), desc=f"Epoch {epoch + 1}/{epochs}", leave=False):
            ...
            with torch.no_grad():
                ...
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/error_prop_1", delta1, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/error_prop_2", delta2, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/error_prop_3", delta3, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/error_prop_4", delta4, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/gradient_1", delta1.T @ xb, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/gradient_2", delta2.T @ h1, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/gradient_3", delta3.T @ h2, epoch)
                writer.add_histogram(f"DFA-4Layer_{num_pdfs}/gradient_4", delta4.T @ h3, epoch)
        ...
        writer.add_scalar(f"DFA-4Layer_{num_pdfs}/train_acc", train_acc, epoch)
        ...
        writer.add_scalar(f"DFA-4Layer_{num_pdfs}/val_acc", val_acc, epoch)
