...
def train_bp(train_loader, val_loader, num_feats, num_pdfs,epochs = 20):
    ...
    for epoch in range(epochs):
            ...
            for name,params in model.named_parameters():
                if params.grad is not None:
                    if name[-4:] == "bias":
                        ...
                        writer.add_histogram(f"BP-3Layers_{num_pdfs}/{name}_grad",params.grad,epoch)
        ...
        writer.add_scalar(f"BP-3Layer_{num_pdfs}",train_acc,epoch)
        # -------- VALIDATION --------
        model.eval()
        val_loss = 0.0
        correct = 0
        ...
        writer.add_scalar(f"BP-3Layer_{num_pdfs}",val_acc,epoch)
        print(
