...
def train_bp(train_loader, val_loader, num_feats, num_pdfs):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in enumerate(tqdm(train_loader, desc=f"Train {epoch+1}/{epochs}", leave=False)):
            ...
            # ✓ CRITICAL FIX: NO torch.no_grad() here - we need gradients for backward!
            logits = model(x)
            loss = criterion(logits, y)

            optimizer.zero_grad()
            loss.backward()  # ✓ Now this will actually compute gradients
            optimizer.step()
            ...
            if batch_idx == 0 and epoch == 0:
                ...
                write_RFA = open(f"BP_flops_{num_pdfs}.txt", "w")
        ...
        with torch.no_grad():  # ✓ Correct: No gradients needed for validation
