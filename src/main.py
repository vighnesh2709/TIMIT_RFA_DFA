...
def main():
    ...
    feature_dir = data_dir / "feature_extracted" / "export_feats_2"

    print("PRE PROCESSING 13 DIMENSIONAL VECTOR\n")
    
    load_mfcc(str(feature_dir / "mfcc_mono.txt"))
    load_alignments(str(feature_dir / "labels_mono.txt"),str(feature_dir/"show-transition.txt"))
    check_dataset()
    write_tensor("13")
    ...
    load_alignments(str(feature_dir / "labels_tri1.txt"), str(feature_dir/"tri-show-transitions.txt"))
    check_dataset()
    write_tensor("39")
    ...
    splice_size = 5
    num_feats_13 = 39 * ((splice_size * 2) + 1)
    num_feats_39 = 117 * ((splice_size * 2) + 1)
    ...
    start_bp = time.time()
    max_acc_train_bp_13, max_acc_val_bp_13 = train_bp(train_loader, val_loader, num_feats_13, 48,epochs)
    end_bp = time.time()
    
    print("BP Bigger model")
    start_bp_v2 = time.time()
    max_acc_train_bp_13_v2, max_acc_val_bp_13_v2 = train_bp_V2(train_loader, val_loader, num_feats_13, 48,epochs)
    end_bp_v2 = time.time()
    print("\n")
    ...
    start_rfa = time.time()
    max_acc_train_rfa_13, max_acc_val_rfa_13 = train_rfa(X, Y, num_feats_13, 48,epochs)
    end_rfa = time.time()
    
    print("RFA Bigger Model")
    start_rfa_v2 = time.time()
    max_acc_train_rfa_13_v2, max_acc_val_rfa_13_v2 = train_rfa_V2(X, Y, num_feats_13, 48,epochs)
    end_rfa_v2 = time.time()
    print("\n")

    print("TRAINING DFA 13 DIMENSIONS\n")
    start_dfa = time.time()
    max_acc_train_dfa_13, max_acc_val_dfa_13 = train_dfa(X, Y, num_feats_13, 48,epochs)
    end_dfa = time.time()
    
    print("DFA Bigger Model\n")
    start_dfa_v2 = time.time()
    max_acc_train_dfa_13_v2, max_acc_val_dfa_13_v2 = train_dfa_V2(X, Y, num_feats_13, 48,epochs)
    ...
    max_acc_train_bp_39, max_acc_val_bp_39 = train_bp(train_loader, val_loader, num_feats_39, 48,epochs)
    end_bp_39 = time.time()
    
    print("BP Bigger model")
    start_bp_39_v2 = time.time()
    max_acc_train_bp_39_v2, max_acc_val_bp_39_v2 = train_bp_V2(train_loader, val_loader, num_feats_39, 48,epochs)
    ...
    max_acc_train_rfa_39, max_acc_val_rfa_39 = train_rfa(X, Y, num_feats_39, 48,epochs)
    end_rfa_39 = time.time()

    print("RFA Bigger Model")    
    start_rfa_39_v2 = time.time()
    max_acc_train_rfa_39_v2, max_acc_val_rfa_39_v2 = train_rfa_V2(X, Y, num_feats_39, 48,epochs)
    end_rfa_39_v2 = time.time()
    print("\n")

    print("TRAINING DFA 39 DIMENSIONS\n")
    start_dfa_39 = time.time()
    max_acc_train_dfa_39, max_acc_val_dfa_39 = train_dfa(X, Y, num_feats_39, 48,epochs)
    end_dfa_39 = time.time()

    print("DFA Bigger Model")
    start_dfa_39_v2 = time.time()
    max_acc_train_dfa_39_v2, max_acc_val_dfa_39_v2 = train_dfa_V2(X, Y, num_feats_39, 48,epochs)
    end_dfa_39_v2 = time.time()
    
    # =====================================================
    # WRITE RESULTS TO FILE
    # =====================================================
    with open("../results/results_random_test.txt", "w") as write_file:
        
        write_file.write("\n" + "="*100 + "\n")
        write_file.write("SUMMARY MONOPHONE DIMENSIONS (144 OUTPUT CLASSES)\n")
        ...
        write_file.write("\n" + "="*100 + "\n")
        write_file.write("SUMMARY TRIPHONE DIMENSIONS (1768 OUTPUT CLASSES)\n")
        write_file.write("="*100 + "\n")
    ...
    print("\nResults saved to ../results/results_random_test.txt")
