...
def train_bp(train_loader, val_loader, num_feats, num_pdfs,epochs = 20):
    ...
    writer = SummaryWriter(f"runs/BP/4layer/{num_pdfs}")
    ...
    # writer = SummaryWriter(f"runs/BP/4layer/{rep}")
    ...
    for epoch in range(epochs):
        ...
        for x, y in tqdm(train_loader, desc=f"Train {epoch+1}/{epochs}", leave=False):
            ...
            #             continue
            #         else:
            #             writer.add_scalar(f"GradNorm/{name}", params.grad.norm().item(), epoch)
            #             writer.add_scalar(f"GradMean/{name}",params.grad.mean().item(), epoch)
            #             writer.add_histogram(f"GradHist/{name}",params.grad,epoch)
            ...
            #             with open("BP_last_layer.txt",'w') as f:
            #                 f.write(params.grad)
            #                 f.write("\n")
            optimizer.step()
    ...
    file_writer(f"BP_4Layer_{rep}_train",train_accs)
    file_writer(f"BP_4Layer_{rep}_val",val_accs)
