...
def train_bp(train_loader, val_loader, num_feats, num_pdfs, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        for x, y in tqdm(train_loader, desc=f"Train {epoch + 1}/{epochs}", leave=False):
            ...
            for name, params in model.named_parameters():
                if params.grad is not None and not name.endswith("bias"):
                    writer.add_histogram(f"BP-4Layer_{num_pdfs}/grad_{name}", params.grad, epoch)
        ...
        writer.add_scalar(f"BP-4Layer_{num_pdfs}/train_acc", train_acc, epoch)
        ...
        writer.add_scalar(f"BP-4Layer_{num_pdfs}/val_acc", val_acc, epoch)
